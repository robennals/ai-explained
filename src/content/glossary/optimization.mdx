...
- **Training a neural network** — measure the [loss](/glossary#loss), calculate the gradient (the direction of fastest improvement), nudge every [parameter](/glossary#parameter) that way, repeat over billions of steps.

[Training](/glossary#training) a [model](/glossary#model) is just optimization applied to its parameters, with the [loss](/glossary#loss) as the number to minimize. The "small change" step uses [gradient descent](/glossary#gradient-descent), which is what makes training fast enough to work in practice — without it, you'd be optimizing by random guesses.
