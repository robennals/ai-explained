...
A **dot product** is the number you get by multiplying matching pairs of numbers from two vectors and adding the results.

For two 2D vectors: `(a, b) · (c, d) = a×c + b×d`. For longer vectors, the pattern is the same — multiply matching numbers, then sum.

What that single number actually means takes a few passes to unpack:
