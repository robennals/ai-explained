...
short: The process of adjusting a model's parameters so it makes the smallest error on the data we give it.
firstAppearance: optimization
---

**Training** is the process of adjusting a [model](/glossary#model)'s [parameters](/glossary#parameter) so it makes the smallest [error](/glossary#error) on the data we give it.
...
In practice, training uses [gradient descent](/glossary#gradient-descent): instead of guessing which parameter to nudge, it calculates which direction shrinks the error fastest, then takes a small step that way. This works only because the model is built to be **smooth** — small parameter changes give small, predictable changes in the error. Training is [optimization](/glossary#optimization) applied to a model's parameters.
