...
An **error** is a number that says how far off something is from perfect on one try.
...
- **A photo classifier** — the answer is "cat", but the [model](/glossary#model) only gave "cat" 30% probability. Error = 100% − 30% = 70%.
...
Averaged across many examples, the error becomes the [loss](/glossary#loss) — the number [training](/glossary#training) tries to make small.
