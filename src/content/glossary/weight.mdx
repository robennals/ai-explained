...
A **weight** is a number inside a [neuron](/glossary#neuron) that controls how much one of its inputs matters.
...
- **GPT-3** — about 175 billion weights, each one nudged during [training](/glossary#training) until the model is good at predicting the next word.

Weights, together with biases, are the [parameters](/glossary#parameter) of a [model](/glossary#model).
