...
export function ToyAttention() {
  ...
  return (
    <WidgetContainer
      title="Toy Attention"
      description={'Click "it" to see what it attends to — every number is visible.'}
      onReset={handleReset}
    >
...
Blend values — weighted sum
                    ...
                    <div key={i} className={`font-mono text-sm ${w < 0.01 ? "text-muted/40" : ""}`}>
                      ...
                      {w > 0.01 && (
                        <span className="ml-1 text-emerald-600 dark:text-emerald-400">
                          → [{tok.value.map((v) => (w * v).toFixed(2)).join(", ")}]
                        </span>
                      )}
                    </div>
                  );
          ...
          <div className="rounded-lg border border-border bg-foreground/[0.02] px-4 py-3 text-center text-sm text-muted">
            Click "it" to see how attention is computed step by step.
          </div>
        )}
