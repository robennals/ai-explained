  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="rounded-lg border border-border bg-foreground/[0.03] p-4 text-base">
        ...
        <div className="border-l-4 border-green-500 bg-green-50 px-3 py-2 text-base text-foreground">
          {finalRep}
        </div>
      </div>

      <div className="rounded-lg border border-border bg-foreground/[0.03] p-4 text-base">
        <div className="mb-3 text-sm font-medium uppercase tracking-wider text-muted">
        ...
        <div className="grid gap-1.5">
          {predictions.map((pred) => (
            <div key={pred.token} className="grid grid-cols-[100px_1fr_50px] items-center gap-2">
              <span className="font-mono text-base font-medium">{pred.token}</span>
              ...
              <span className="text-right text-base font-medium text-muted">
                {Math.round(pred.probability * 100)}%
              </span>
            </div>
          ))}
        </div>
        <div className="mt-3 text-sm italic text-muted">
