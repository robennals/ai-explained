...
So far we've talked about "word embeddings," but modern language models don't actually split text by whole words. They use **tokens**: chunks that can be full words, pieces of words, punctuation, or spaces.

Why? If every distinct word needed its own token, vocabulary size would explode. English alone has huge numbers of rare forms, misspellings, names, and invented words. A model would waste capacity memorizing one-offs.

Subword tokenization fixes this. Instead of storing a separate entry for every word, the model learns reusable pieces. A word like **walking** might be split into pieces like **walk** and **ing**. Then if it sees **moonwalking**, **sleepwalking**, or a rare new word, it can still reuse chunks it already understands.

Two common methods are **BPE** and **WordPiece**. They both build reusable subword chunks, so words like `walking`, `walked`, and `walker` can share pieces. That keeps vocabularies manageable and helps models handle rare or new words.

<TokenizationPlaygroundWidget />

<TryIt>
Click "walking walked walker" and look for repeated parts. Then try a made-up word like "qwertyflorp" and notice it still gets tokenized into known chunks. Add punctuation and spaces to see that they are tokens too.
</TryIt>
