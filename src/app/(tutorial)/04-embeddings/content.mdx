...
In the previous chapter, you built neural networks that classified 2D points. Those points were abstract dots on a grid. But now each dot has a name — its position in the embedding encodes meaning. Can a neural network learn to read that meaning?

Let's find out. Below is the same kind of two-layer network, but now the two inputs are a word's Size and Danger coordinates from the scatter plot above. Each word sits at its embedding position, and the network has to learn which words match a concept.

<EmbeddingClassifierWidget />

<TryIt>
Start with "Dangerous" — the network needs to separate the top half (bear, shark, knife, gun) from everything else. Hit Optimize and watch it learn. Then try "Big" — it draws a vertical boundary instead. "Big & Dangerous" is harder: only bear and shark qualify, so the network must combine BOTH neurons to carve out the top-right corner. Try all four challenges and see how the decision boundary changes shape for each concept.
</TryIt>
