The big idea behind AI is that a sufficiently large model can compute pretty much anything, provided it has been configured with the right parameters. But how do you find the right parameters? The answer is to use the same method that drives evolution: make a tiny change, check whether it made the model work better, keep it if it did, and repeat this process very very fast, many many times.

## How Do You Build Something You Don't Understand?

A model is a machine with adjustable knobs. Different knob settings compute different functions. We need to find the settings that make the model compute the *right* function, but we can't try every combination, because the number of possibilities is astronomically large.
...
Switch between examples and drag the slider. Every example has the same structure: there's a current value and an error that measures how far off it is. Making the error smaller is what optimization means.
</ErrorMeasurementWidget>
...
Start in "Blind" mode and try to find the hidden target. All you learn from each guess is "not here". A little ✗ appears and you have zero idea whether your error is going up or down. After a dozen guesses, hit "Reveal target" and see how hopeless it was. Then switch to "With error signal." Now each dot's size and color tells you how big the error is, and you're constrained to small steps near your last guess. Watch how quickly you zero in. Same problem, but the error signal makes all the difference.
</OptimizationGameWidget>
...
Click anywhere to drop a ball. On the smooth curve, it rolls to the bottom. On the stepped version, it just sits wherever it lands, with no slope to follow.
</SmoothVsRuggedWidget>
...
In <Ch slug="computation" />, we saw that a model is a machine with adjustable knobs. The question was: how do you find the right settings when there are millions of knobs?
...
But what should this model actually *look like*? What's inside a neural network? In <Ch slug="neurons" />, we'll meet the building block: the **artificial neuron**, a simple unit that takes a weighted sum of its inputs, adds a bias, and passes the result through a nonlinear function. It's almost embarrassingly simple. But stack enough of them together, and they can compute anything.
