How do you teach an AI the sum of all human knowledge? It turns out that one of the most effective ways is to train it to be really good at predicting the next word of an arbitrary piece of human-written text. This chapter explains why.
...
It does, but we hit a wall. The same wall we saw back in <Ch slug="computation" /> with the lookup table explosion.
...
That's the idea behind **attention**, and it changed everything. In <Ch slug="attention" />, we'll see how it works, built from pieces you already know: embeddings, dot products, and neural networks.
