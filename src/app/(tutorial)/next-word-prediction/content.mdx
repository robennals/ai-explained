We can now turn words into vectors. The question is what to train a model to do with them. The answer that produced modern AI sounds much too simple: guess the next word. A strange claim comes attached to it. Anything that predicts the next word accurately enough has to *understand* the sentence it is predicting.
