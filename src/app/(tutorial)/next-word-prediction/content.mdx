Modern language models are trained to do one thing: guess the word that comes next. It sounds far too simple to produce anything interesting. A strange claim comes attached to it, though. Anything that predicts the next word accurately enough has to *understand* the sentence it is predicting, and by the end of this chapter you should see why that is true.
...
It does, but we hit a wall. The same wall we saw back in [Computation](/computation) with the lookup table explosion.
