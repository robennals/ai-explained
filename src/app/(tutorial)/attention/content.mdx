...
Understanding a sentence requires many kinds of questions at once: What does this pronoun refer to? What's the nearest noun? What's the subject of this verb? What clause does this word belong to?

Real attention models use **multiple attention heads** — each with its own key, query, and value weights, each learning to look for different kinds of relationships. They all run in parallel on the same input, and their outputs get combined.

Earlier we saw a single BERT head that learned pronoun resolution. Here's the same model, but now showing four of its heads — including two that use positional information:

<BertAttentionWidget>
Switch between the four heads. "Next word" and "Previous word" are position-based — every word rigidly attends to its neighbor, building a chain of local context. Compare with "Self / pronoun" and "Broad context" which attend based on meaning. All four run in parallel inside the same model.
</BertAttentionWidget>

The "Next word" and "Previous word" heads only work *because* the model has positional encoding telling it where each word sits. Without RoPE (or a similar mechanism), the model couldn't tell which word is adjacent — it would only know what the words mean, not where they are. No one told these heads what to specialize in — they discovered their roles through training.
