...
Given a token, we can work out which other tokens it should be paying attention to by multiplying its query (what it is looking for) by the key of each of the other tokens. This is the [**dot product**](/vectors) from the Vectors chapter. The bigger the score, the better the match.
...
The answer is the same answer we've run into many times before. The model uses [gradient descent](/optimization) to learn the [weights](/neurons) that make the model perform best and thus minimize its error.

To be more precise, the model learns a simple one-layer [neural network](/neurons) (without an activation function) that transforms the token's current [embedding](/embeddings) into query, key, and value vectors.
