Attention is the breakthrough that made modern AI work, and the idea behind it is simple. When a model processes a word, it should be able to reach back and pick out whichever earlier words give that word its meaning, however far back they sit. This chapter builds attention from scratch, out of <Ch slug="vectors">vectors</Ch>, dot products and <Ch slug="neurons">neural networks</Ch>.
...
This is something our fixed-window model from <Ch slug="next-word-prediction" /> can't do well. It sees the last few words and has no way to reach back and pick out the *specific* earlier words that matter.
...
Given a token, we can work out which other tokens it should be paying attention to by multiplying its query (what it is looking for) by the key of each of the other tokens. This is the **dot product** from <Ch slug="vectors" />. The bigger the score, the better the match.
...
But our attention mechanism has no idea *where* words are — only *what* they are. "The dog chased the cat" and "the cat chased the dog" produce identical attention scores. In <Ch slug="positions" />, we'll fix this with a trick involving rotation.

<TryItInPyTorch notebook="attention">
Compute dot-product attention from scratch, build query/key/value projections, visualize attention heatmaps on real sentences, implement multi-head attention, and see how scrambled word order doesn't change attention scores, a problem we'll solve in <Ch slug="positions" />.
</TryItInPyTorch>
