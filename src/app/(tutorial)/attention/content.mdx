...
The toy example uses **softmax** to turn raw dot-product scores into attention weights. Each token has a fixed budget of attention — 100% — and softmax decides how to divide it.

The dot products give us raw scores — but they can be any numbers: large, small, negative, zero. Softmax converts them into positive weights that add up to 100%. Think of it as a competition: what matters is **who's loudest relative to everyone else**. If one score is moderately high and the others are near zero, it wins by default. But if another score comes along much higher, it drowns out the moderate one and takes nearly all the weight. The *gaps* between scores matter more than the absolute values.
...
So far attention tells us *where to look* — which tokens are relevant. But we also need to know *what information to gather* from those tokens. That's what the **value** vector is for.

In our pronoun-resolution example, the question is "what thing does *it* refer to?" Each token publishes a **value** — the answer to whatever question the attention head is asking. For our toy example, the value encodes which animal the token represents: cat's value says "cat," dog's value says "dog," and filler tokens have nothing to contribute.
