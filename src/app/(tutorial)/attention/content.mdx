Predicting the next word requires understanding, but the models so far can only see a fixed window of recent words, with no way to reach further back for the one word that matters. What if each word could *choose* which earlier words to look at? That idea is called attention, and it is the breakthrough that makes modern AI work. It is built entirely out of pieces already covered.
