...
- A **value** — what this token *provides* when attended to
...
To measure how well a query matches a key, we use the **dot product** from Chapter 4 — multiply corresponding numbers and add them up. A high dot product means a good match. Then we run the scores through a function called **softmax** (described soon) that those scores into positive numbers that add up to 100%.
...
In practice, real attention vectors have dozens or hundreds of dimensions, not just one. But the principle is the same — remember the "bear detector" from the vectors chapter, where a single dot product could check for a complex things like "something similar to a bear". Attention queries and keys work the same way, just asking richer questions like "is this a noun that's the subject of a nearby verb?"
