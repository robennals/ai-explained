...
In 1936, two mathematicians, Alan Turing and Alonzo Church, independently came up with two models, the *Turing machine* and the *lambda calculus*, that they claimed could be set up to compute any function that can be computed at all. Not just math. Anything. This claim is called the *Church-Turing thesis*. It can't be formally proven, but no one has ever found a function their models can't handle, and the many other models invented since appear to compute anything too, given the right parameters.

The models Turing and Church came up with were quite different from the ones we use today in AI. In particular, the knobs you set were descriptions of computer programs, rather than lists of numbers. But the same idea applies: choose the settings and you choose the function.

So if "thinking" is computation (turning numbers into numbers), then a computer can in principle do anything a human mind can do. The question isn't whether the right function exists. The question is whether we can build it.

The simple cubic model we gave above is very limited in what it can compute, but there are other models, like the neural networks we'll discuss later, that are universal: they genuinely can compute anything. You just need to find the right settings for their parameters.
