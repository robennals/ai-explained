Modern AI does things that would have seemed impossible for a computer a few years ago. Underneath all of it is math, and less math than you would expect. This chapter is about what kind of math, and why a foundation this simple can do so much.

## What Can AI Do?

A few years ago, the things AI does routinely now would have read as science fiction. It can translate between hundreds of languages, generate a photorealistic image from a description, write complex computer programs from scratch, or perform expert tasks better than most experts. The goal of this tutorial is to give you a deep intuitive sense of how AI does these things. 
...
So all of AI reduces to one problem: build the right function.

If you could build the function that maps photo pixels to the word "cat", that's image recognition. If you could build the function that maps English to French, that's translation. If you could build the function that maps prompts to good answers, that's a chatbot.

But how do you build a function?

## Functions with Knobs

The trick is to start with a *model*: a general purpose function whose behavior you can change using adjustable knobs called *parameters*. Change the knob settings and you get a function that does something different. 

Here's a concrete example. The model below has four parameters (a, b, c, d) and computes:

**y = a + b·x + c·x² + d·x³**

There's a target curve (the blue one) we want the model to match. Your job is to adjust the four knobs so the red curve sits on top of the blue one. The error shows how different the curves currently are.
...
<KeyInsight>
**A model is a function with adjustable knobs.** Different settings compute different functions, and building the function you want becomes a matter of finding the right knob settings. With four knobs, hand-tuning is already frustrating. Real AI models have *billions*. We'll need a way to set them automatically.
</KeyInsight>

## Can One Model Compute Everything?

Before going further, a question worth asking: is it possible to create a model that can build any function we might possibly want?

In 1936, two mathematicians, Alan Turing and Alonzo Church, independently came up with two models, the *Turing machine* and the *lambda calculus*, that they claimed could compute any function that can be computed at all. Not just math. Anything. This claim is called the *Church-Turing thesis*. It can't be formally proven, but no one has ever found anything computable that these models cannot. We call a model that can compute any computable function a *universal model*.

The simple cubic model we gave above is not a universal model, and is very limited in what it can compute. But some of the models we use in AI are universal models. If you allow them to become big enough, and run long enough, and find the right parameters, they really can compute anything that can be computed.

So if "thinking" is computation (turning numbers into numbers), then a computer can in principle do anything a human mind can do. The question isn't whether the right function exists. The question is whether we can find the right parameters.
...
3. **Models are functions with knobs.** Different settings compute different functions.
...
In <Ch slug="optimization" /> we'll see the surprising answer: you don't need to work out the knob settings by hand. You only need a way to make reliable tiny improvements, and then to make a lot of them, very fast. The same algorithm behind biological evolution, A/B testing, and the scientific method can find the right parameter values automatically. It's called *incremental optimization*, and it's the engine behind all of modern AI.
