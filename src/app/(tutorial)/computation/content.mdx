...
It's worth being clear about who does what, because it's a common point of confusion. *Designing the model* (deciding what knobs the machine has and how they combine the input) is (currently) a human job. The automatic process doesn't invent the function or pick it from a catalogue of every possible function. It only turns the knobs on a machine a person already built. A human chose the cubic shape above; only then could tuning a, b, c, and d make it fit.

So where do good models come from? At least for now, people design them. Much of the rest of this tutorial tours the designs that turned out to work remarkably well: neural networks, then attention and the transformer. Optimization tunes a model; it doesn't invent one.
