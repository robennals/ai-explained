...
To measure how well a query matches a key, we use the **dot product** from Chapter 5 — multiply corresponding numbers and add them up. A high dot product means a good match. Then we run the scores through **softmax**, which turns them into percentages that add up to 100%.

<Callout>
**Softmax** is the function that turns raw dot-product scores into attention weights. It takes a list of numbers and converts them into percentages that add up to 1 (or equivalently, 100%). Bigger scores get disproportionately more weight — softmax amplifies the differences, so the model focuses sharply on the most relevant words rather than spreading attention evenly. It's used throughout deep learning whenever you need to turn scores into a probability distribution.
</Callout>
...
## Values: What You Actually Take Away
...
<KeyInsight>
Attention is just three steps we already understand: (1) each word gets a query, key, and value vector, (2) queries and keys are compared with dot products (Chapter 5) and passed through softmax to compute attention weights, and (3) values are blended together using those weights. **No new math — just a clever wiring of familiar pieces.**
</KeyInsight>
