Pretty much every modern AI system is a neural network. Neurons are a bit like the digital logic gates that make up a computer, except that their output changes smoothly rather than snapping on and off, and it is that smoothness that allows the network to be optimized through training.
