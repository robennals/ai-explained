Pretty much every modern AI system is a neural network. Each neuron by itself is extremely simple, but if you wire enough of them together, and give them the right parameters, they can do amazing things.
...
Click the different neurons to see what each type does. Don't worry if it doesn't all make sense yet. The rest of this chapter will explain how it works.
</NetworkOverviewWidget>
...
The key property for neural networks: when you change a weight by a small amount, the output changes by a small amount. No sudden jumps, no unpredictable flips. That's exactly what <Ch slug="optimization" /> told us we need for optimization to work.
...
But logic gates have a problem. Their outputs snap between 0 and 1 with nothing in between. There's no smooth gradient to follow, which means you can't train a network of logic gates using the optimization techniques from <Ch slug="optimization" />. You can't ask "how should I adjust this gate to make the answer a *little* more correct?" It's all or nothing.
...
Click the **AND**, **OR**, **NOT**, and **NAND** buttons above and watch the weights smoothly shift to compute each gate. Then try sliding the weights yourself. You can smoothly blend between different logic operations.
</NeuronPlaygroundWidget>
...
XOR needed a single hidden layer computing intermediate facts. Harder problems need many. You can't jump straight from raw pixels to "that's a cat," so you build up through stepping stones. The first hidden layer might detect simple patterns: edges, color patches, textures. The next combines those into parts: eyes, ears, fur. A later layer combines *those* into whole objects: "cat" or "dog." Real networks have lots of layers, allowing them to think very deeply. GPT-3 has 96 layers and frontier models today likely have more.

No human decides what new things each layer computes. The AI learns that as part of its training. So how do we train weights buried deep inside the network? Surprisingly straightforwardly. Every weight contributes to the final output, so it contributes to the error, which means every weight has a **gradient** (as we saw in <Ch slug="optimization" />) telling us which direction to nudge it. We run gradient descent on **every weight at once**, using an algorithm called **backpropagation** that computes those gradients efficiently by flowing the error signal backward through each layer.
