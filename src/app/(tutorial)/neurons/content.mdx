Pretty much every modern AI system is a neural network. The name evokes brain cells, but the part they are built from is closer to a logic gate, one of the AND/OR/NOT components inside every computer chip. The important difference is that a neuron's output slides smoothly instead of snapping between on and off, and that smoothness is what makes a network trainable. Nudge a weight and the answer gets slightly better or slightly worse, which is exactly what [optimization](/optimization) needs. This chapter explains what neural networks are, builds one from scratch, and shows how it learns.
