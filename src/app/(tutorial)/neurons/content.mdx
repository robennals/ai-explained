...
Drag the sharpness slider all the way up. The curve snaps almost like a light switch — 0 or 1, nothing in between. Now drag it down. The curve becomes a gentle slope. Very sharp neurons act like logic gates, but they're harder to train because the gradient nearly vanishes on the flat parts.
</SharpnessExplorerWidget>
...
Wire enough gates together and you can build circuits that do real computation. The circuit below uses just 10 gates to add two 2-bit numbers — it can compute 3 + 2 = 5, for example. Each "full adder" block handles one digit of the addition, and the carry output of one feeds into the next, just like carrying in long addition by hand.

<GateCircuitDiagramWidget />

This is a tiny circuit, but the principle scales. Real computer chips assemble **billions** of gates on a single chip. That's enough to be a **universal computer**: a machine that can compute anything that can be computed, given enough time and memory. Your phone, your laptop, every digital device works this way — just gates, wired together.
