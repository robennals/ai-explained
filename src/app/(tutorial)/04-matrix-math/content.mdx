...
<div className="flex items-center gap-1"><span className="text-xl text-muted">[</span> <span className="w-8 text-center">a</span> <span className="w-8 text-center">b</span> <span className="text-xl text-muted">]</span></div>
<div className="flex items-center gap-1"><span className="text-xl text-muted">[</span> <span className="w-8 text-center">c</span> <span className="w-8 text-center">d</span> <span className="text-xl text-muted">]</span></div>
</div>
</div>

The first column is &ecirc;&sub1; — it tells you where the x-axis basis vector ends up. The second column is &ecirc;&sub2; — it tells you where the y-axis basis vector ends up. Everything else — every single point in the entire plane — just follows along.
...
<TryIt>
Switch to the **Cat** and try the presets. Hit **Rotate 90&deg;** — the cat spins a quarter turn. **Shear** — the cat leans over like it's dodging something. **Reflect** — mirror-image cat! **Collapse** — the cat gets squished into a line (poor cat). Now switch to the **Mona Lisa** and try **Rotate 45&deg;** — can you tell she's tilted? Try making your own transformation: can you make the Mona Lisa twice as wide but the same height? (Hint: in the red box, drag the x slider to 2 — you're telling the x-axis basis vector to stretch to twice its length.)
</TryIt>

Watch the red and green arrows in the widget — those are the basis vectors. The red arrow (&ecirc;&sub1;) shows where (1, 0) ends up after the transformation. The green arrow (&ecirc;&sub2;) shows where (0, 1) ends up. This is exactly what we saw in 1D: the transformation is completely determined by where the basis vectors land. In 1D we had one basis vector and one number. In 2D we have two basis vectors and four numbers.
...
## Why Matrices? Why Not Just Say "Rotate and Scale"?

If every 2D matrix is "just" some combination of rotation, scaling, and shearing... why bother with matrices at all? Why not just say "rotate 30&deg;, scale 2&times;, shear by 0.3"? That seems more intuitive.

Three reasons make matrices the right choice:

**1. Matrices compose by multiplication.** Want to apply transformation A, then transformation B? Just multiply: **C = BA**. The result is another matrix of the same shape — one operation, clean and simple. With rotation-scale-shear, composing two transforms requires messy trigonometric formulas that don't simplify.

**2. Learning is simpler.** Neural networks need to *learn* what transformation to apply. With a matrix, the network adjusts four independent numbers. With rotation-scale-shear, the parameters interact nonlinearly — a tiny change in rotation angle can cause a big swing in the result. Independent parameters are much easier to optimize with gradient descent.

**3. Matrices are more general.** A matrix can collapse entire dimensions (det = 0), project space onto a line, or do things that don't decompose neatly into rotation-scale-shear at all. Neural networks need that full generality.

<Callout type="tip" title="Same math, different field">
This exact same matrix math powers 3D graphics in video games. GPUs — the chips designed to rotate, scale, and shade triangles in games — turned out to be exactly what neural networks needed. This isn't a coincidence. Both fields need to do massive numbers of matrix multiplications very fast.
</Callout>
...
<TryIt>
Try different models — the **Sphere** makes stretching and squishing super obvious, and the **House** makes rotation easy to see. Each colored box below the matrix controls one basis vector: the red box sets where the X-axis lands, the green box sets where the Y-axis lands, and the blue box sets where the Z-axis lands. Try the presets: **Rot X 45&deg;** tilts the object forward, **Flatten Y** crushes it flat like a pancake, and **Shear XY** makes it lean sideways. Can you combine changes to make the house spin AND stretch? (Hint: start from a rotation preset, then drag the sliders to make each basis vector longer.)
</TryIt>
