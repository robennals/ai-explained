...
There's a powerful second way to read a matrix. Instead of thinking "where do the old basis vectors end up?" (the columns), think about the **rows**: each row defines a new measurement direction.

The <span style={{color: "#ef4444"}}>first row</span> says: "to find a point's new x-coordinate, take the dot product with *this* vector." The <span style={{color: "#22c55e"}}>second row</span> does the same for the new y-coordinate. You're defining a new coordinate system by choosing new directions to measure along.

The colored lines below are **iso-lines** — lines where one new coordinate has a constant value. On the left, you see the new coordinate grid overlaid on the original space: <span style={{color: "#ef4444"}}>red lines</span> show where the new x-coordinate is constant, <span style={{color: "#22c55e"}}>green lines</span> show where the new y-coordinate is constant. On the right, those same coordinates become the standard grid — red vertical lines and green horizontal lines. Each labeled point sits between the same pair of iso-lines in both views.

<BasisVectorViewWidget />

<TryIt>
Start with **Identity** — the colored iso-lines match the standard grid and nothing changes. Now try **Shear** — the red iso-lines tilt, so the new x-coordinate includes some of the old y. Watch how the points slide horizontally but their y-coordinates stay the same. Try **Rotate 90&deg;** — both sets of iso-lines rotate, and the shape rotates to match. Try **Collapse** — both rows become the same, so the red and green iso-lines overlap and all points squish onto a single line.

Now try dragging just the "old y" slider in the red row. The red iso-lines tilt, and on the right you can see each point's new x-coordinate shift based on how much old-y it has. Points with higher old-y values get pushed further right.
</TryIt>

This dual interpretation — "where do old axes land?" (columns) vs. "what are my new measurement directions?" (rows) — is the same math read two ways. Both are useful. In neural networks, the row view is especially natural: each output neuron computes a dot product with one row of the weight matrix, measuring how much the input aligns with that row's direction.
