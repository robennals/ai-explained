...
<TryIt>
Start with "Categories" — notice how animals cluster apart from vehicles and food. Switch to "Size × Danger" to see two continuous properties at once. Then try "Animal or Food?" — chicken, salmon, and lamb appear in the overlap because they belong to both categories.
</TryIt>

Two dimensions are dramatically more expressive than one. With just two numbers, we can separate categories that were hopelessly mixed on a single line. We can plot two independent properties. We can even represent overlapping categories like a Venn diagram. But we still can't distinguish everything — dog and cat overlap, guitar and flute sit on top of each other. We'd need more dimensions: "is it a pet?", "does it have strings?", "is it edible?"
