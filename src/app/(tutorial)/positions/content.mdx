The meaning of a sentence often depends on the *order* of its words, not just on which words are there. The basic [attention](/attention) mechanism is blind to word order, so we need to extend it a bit if we want our AI model to take word order into account.
