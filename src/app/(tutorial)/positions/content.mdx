The meaning of a sentence often depends on the *order* of its words, not just on which words are there. The basic <Ch slug="attention">attention</Ch> mechanism is blind to word order, so we need to extend it a bit if we want our AI model to take word order into account.
...
The attention system we built in <Ch slug="attention" /> *can't tell any of these apart*.
...
Attention now knows three things: what each word *means* (from embeddings), which other words *matter* (from attention scores), and where each word *is* (from rotation). In <Ch slug="transformers" />, we'll put all of these pieces together to build the **transformer**, the architecture behind ChatGPT, Claude, and most modern language models.
