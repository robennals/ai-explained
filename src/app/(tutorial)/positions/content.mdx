...
Drag the slope around. At slope 0, both nouns get identical attention, so the model is position-blind. Higher slopes make the closer noun ("cat") win more decisively over the farther one ("dog").
</ALiBiToyTokensWidget>
...
Start with rotation speed at 0°. Both nouns get identical attention, just like in <Ch slug="attention" />. Now drag it up. The closer noun ("cat") starts winning because its key is rotated to a more similar angle to "it"'s query. This achieves the same distance preference as ALiBi, but through geometry rather than an explicit penalty.
</RotationToyTokensWidget>
