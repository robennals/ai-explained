...
Pick an item and drag the slider to change the magnitude. The dashed arrow is the unit vector — it sits on the arc where all unit-length vectors live. The faded arrow shows the full vector scaled by the count. Try switching between Animals, Speed, and Colors to see how "direction = what kind, magnitude = how much" applies everywhere.
</AnimalDirectionMagnitudeWidget>
