...
This idea — building up complex descriptions by adding simpler vectors together — turns out to be fundamental to how neural networks work. Deep inside a real model, every layer *adds* its contribution to a running sum, gradually refining what each token represents. The whole architecture depends on the fact that vectors can be combined by addition.
