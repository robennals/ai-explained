The word "vector" sounds more intimidating than it is. A vector is a list of numbers, nothing more. What makes it worth a chapter is that a list of numbers can describe almost anything, from a colour to an animal to the meaning of a word, and that understanding vectors makes it much easier to understand <Ch slug="neurons">neural networks</Ch>.
...
Note that if two animals are zero in a dimension then that dimension contributes nothing. That is because we think of each dimension as being something **notable** about the animals. "Hairy = 1" means hair is one of this animal's defining features. "Hairy = 0" means hair isn't really worth mentioning. If we cared about what animals were notably hairless, we could add a "hairless" dimension, or have negative values of "hairy".
...
This idea, building up complex descriptions by adding simpler vectors together, turns out to be central to how neural networks work. This is something we'll return to in <Ch slug="embeddings" /> and again in <Ch slug="transformers" />.
...
Remember from <Ch slug="neurons" /> that a neuron computes a **weighted sum** of its inputs, adds a bias, then applies an activation function. That weighted sum is a dot product. The neuron has a **weight vector** and receives an **input vector**:
...
### Bias and Magnitude Do Different Jobs

You just had two knobs to play with, bias and weight magnitude, and they can feel like they do the same thing: both make the neuron fire on more or fewer inputs. But they reshape the neuron's response in different ways. To see this, plot the output against *how bear-like the input is*.

The **magnitude** sets how *steeply* the curve rises while the **bias** *slides* the curve sideways.

<NeuronResponseCurveWidget>
Drag the bias: the whole curve slides sideways, moving the decision boundary while the steepness stays put. Now drag the weight magnitude: the curve steepens, and notice the boundary drifts too. That's the real difference between them: bias only moves the boundary, while magnitude *also* changes how sharp the switch is. Try the presets to jump between a wishy-washy detector and a hard, decisive one.
</NeuronResponseCurveWidget>
...
This geometric picture will return later, especially in <Ch slug="attention" /> and <Ch slug="positions" />.
...
In <Ch slug="embeddings" />, we'll see how AI solves this problem: instead of choosing dimensions by hand, it *learns* vectors where similar words end up nearby and directions encode meaningful relationships, all without any human labeling.
