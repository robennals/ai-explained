...
Think of velocity: the direction tells you *where* you're heading, and the length tells you *how fast*.

The way we represent direction as a vector is with a **unit vector** — a vector of length 1 that points in that direction. Any vector equals its unit vector times its length.

<DirectionMagnitudeExplorerWidget>
All three sets of sliders control the same vector — try any of them and watch the others update. Direction + Length controls the angle and length. Unit Vector + Length shows the unit vector (always length 1 on the grid — notice its tip stays on the dashed circle) scaled by the length. Raw Vector lets you set x and y directly.
</DirectionMagnitudeExplorerWidget>

This decomposition — direction times length — will matter a lot in a moment, when we look at what the dot product really means.

## The Dot Product

The **dot product** is the single most important vector operation in AI. It's a way of multiplying two vectors together to get a single number. That number tells you two things at once: how big the vectors are, and how much they point in the same direction. 

For regular numbers (one-dimensional vectors), the dot product is just ordinary multiplication: 3 · 5 = 15. The dot product extends this idea to vectors with more dimensions.

There are two ways to compute the dot product, and they always give the same answer.
...
**Way 2 — projection:** Multiply the lengths of the two vectors, and then multiply that by how much the two vectors point in the same direction (measured using a function called `cosine`).
...
When both vectors are unit vectors (length 1), the dot product simplifies to just the cosine — pure directional agreement with length factored out. This is called **cosine similarity**, and we'll see it again when we get to embeddings.
...
output = sigmoid(w₁ × x₁ + w₂ × x₂ + … + wₙ × xₙ + bias)

That weighted sum is a dot product. The neuron has a **weight vector** w = (w₁, w₂, …, wₙ) and receives an **input vector** x = (x₁, x₂, …, xₙ), and it computes their dot product. We can write the whole thing as:
