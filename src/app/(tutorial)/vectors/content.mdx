A neuron takes in a list of numbers and produces one number out. Once a layer has hundreds of neurons in it, tracking those numbers one at a time stops being useful, so it helps to have a name for the whole list. The name is *vector*, which sounds more intimidating than it is. It is the key to seeing what a neuron actually does, and why stacking neurons in layers works at all.
