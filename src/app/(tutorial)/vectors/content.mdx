...
Design your own animal — notice how adjusting one property automatically scales the others to keep the magnitude at 1.
</UnitVectorExplorerWidget>
...
**Modifying.** You can also add a vector that pushes in a specific direction. Start with a bear and add "more cuddly, less scary, smaller" — you get something like Paddington Bear. Start with a dog and add "bigger, scarier" — you get a wolf. The result describes something that wasn't in our original list.
...
This idea — building up complex descriptions by adding simpler vectors together — turns out to be fundamental to how neural networks work. In a later chapter, we'll see that deep inside a neural network, information flows through a running sum called a **residual stream**, and each layer *adds* its contribution to this stream. The entire architecture depends on the fact that vectors can be combined by addition.

We'll also see in the next chapter that learned word vectors have a famous property: **king − man + woman ≈ queen**. The direction from "man" to "woman" captures something about gender, and you can add that direction to other words. Vector addition isn't just a mathematical curiosity — it's one of the core operations of AI.
