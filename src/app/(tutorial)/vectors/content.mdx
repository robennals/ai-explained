...
You already use vectors every day. Your GPS location is two numbers: (latitude, longitude). An RGB color is three numbers: (red, green, blue). A velocity is two numbers: (east/west speed, north/south speed). An RPG character might have stats like (health, strength, speed). Each of these is a vector — a list of numbers where each slot means something specific.
...
The key idea: a vector is a compact way to describe something as a list of measurements. The number of measurements is the vector's **dimension** — a GPS coordinate is 2-dimensional, an RGB color is 3-dimensional.
...
With just one number, each animal is a point on a line. Let's place them by a single property.

<Vector1DExplorerWidget>
Switch between properties. On "Big," elephant and bear are far apart from mouse and rabbit — but cat and eagle end up right next to each other despite being nothing alike. On "Scary," shark and bear are neighbors even though one lives in the ocean and the other in a forest. One dimension captures one thing but conflates everything else.
</Vector1DExplorerWidget>

## Two Dimensions

With two numbers, each animal is a point on a plane. Now we can capture two properties at once.

<Vector2DExplorerWidget>
Start with Big x Scary. Bear is big AND scary — upper right. Rabbit is small AND not scary — lower left. Shark and bear are both scary but you can tell them apart by which *kind* of scary. Try switching axes — every combination reveals different clusters. Notice how animals that were stuck together in 1D get pulled apart in 2D.
</Vector2DExplorerWidget>

Two dimensions are much more expressive than one. With two numbers, we can distinguish things that a single number couldn't separate.
...
Three dimensions is the most we can directly visualize. But our animal vectors have *six* properties — they really live in 6D space. We can't draw that, but the geometric intuition still works: **nearby points are similar, far-apart points are different, and adding more dimensions lets you make finer distinctions.** This intuition will carry us through the rest of AI.

## Direction and Magnitude

There's another way to think about a vector besides "a list of numbers." Any vector can be split into two parts:

- **Direction**: which way it points (the **unit vector** — a vector of length 1)
- **Magnitude**: how long it is

Think of velocity: the direction tells you *where* you're heading, and the magnitude tells you *how fast*. Or think of color intensity: the direction gives you the *hue*, and the magnitude gives you the *brightness*.

<DirectionMagnitudeExplorerWidget>
In free-drag mode, drag the arrow anywhere. In Direction + Magnitude mode, the direction is locked to the unit circle and you control the magnitude separately with the slider. Notice how the dashed unit vector always points the same way as the full vector — it's the vector's "pure direction" with the length stripped away.
</DirectionMagnitudeExplorerWidget>
...
The **dot product** is the single most important vector operation in AI. It takes two vectors and produces a single number. There are two ways to compute it, and they always give the same answer.

**Way 1 — multiply the parts and add:** Multiply each pair of corresponding numbers, then add the results. For vectors (a₁, a₂) and (b₁, b₂): dot product = a₁ × b₁ + a₂ × b₂.

**Way 2 — projection:** Multiply the magnitudes of both vectors by the cosine of the angle between them: |a| × |b| × cos(θ).

<DotProductExplorerWidget>
Drag the arrow tips. In Component view, watch the individual products and their sum. Switch to Projection view to see the angle and the shadow of one vector onto the other. Rotate one vector around — when they point the same way, the dot product is large and positive. When perpendicular, it's zero. When opposite, it's negative. Both views always show the same number.
</DotProductExplorerWidget>

The dot product measures **how much two vectors agree**. This is why it appears everywhere in AI: whenever a network needs to ask "how similar are these two things?", the answer is a dot product.

When both vectors are unit vectors (length 1), the dot product simplifies to just cos(θ) — pure directional agreement with no magnitude involved. This is called **cosine similarity**, and we'll see it again when we get to embeddings.
...
Here's the connection that ties everything together. Remember from the previous chapter that a neuron computes a **weighted sum** of its inputs, adds a bias, then applies the sigmoid:

output = sigmoid(w₁ × x₁ + w₂ × x₂ + bias)

That weighted sum — w₁ × x₁ + w₂ × x₂ — is a dot product. The neuron has a **weight vector** (w₁, w₂) and receives an **input vector** (x₁, x₂), and it computes their dot product.

What does this mean? The neuron is asking: **"how much does this input point in the direction of my weights?"** When the input vector aligns with the weight vector, the dot product is large, the sigmoid outputs close to 1, and the neuron fires strongly. When they're perpendicular, the dot product is near zero. When they point in opposite directions, the neuron stays quiet.

<NeuronDotProductWidget>
Adjust the weight and input vectors. Watch the angle between them — when the vectors align (small angle), the output is high. When they're perpendicular (90°), the output depends mostly on the bias. When they oppose each other (large angle), the output drops. Try making the bias very negative — now the vectors need to be *very* well aligned for the neuron to fire. The neuron is a direction detector.
</NeuronDotProductWidget>
