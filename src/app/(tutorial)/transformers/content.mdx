...
The shape of that wiring is simple: a stack of layers, applied one after another to your sentence. At each layer, every word looks at the other words it cares about, pulls in information from them, and uses what it pulled in to refine its own meaning. The first layer might just notice "this word is a noun." Layers above figure out "this noun is the subject of the sentence," then "this subject is a character in a story." Stack enough layers and the words have built up an understanding rich enough to predict what comes next.

This chapter shows that idea at three zoom levels: the whole stack at a glance, one sentence flowing through it in detail, and finally a close-up of one layer.

## A Transformer At a Glance

The picture below is a transformer doing its work on a tiny sentence. The bottom row is the words you typed in. Each row above is what those words look like after one more layer has run. **Amber arrows** are each word's own information passing up unchanged. **Blue arrows** are attention — at every layer, every word picks which earlier words to listen to and pulls information from them. Blue arrows only ever lean rightward: a word can never look at the future.
...
The picture above showed the *shape* of the wiring — words sitting in rows, layers stacked above, attention flowing between them. What it didn't show is what each cell actually *means*. The playground below fills that in. Every box now carries the model's understanding of that word at that layer, written out in plain English, so you can watch the meaning of each word grow richer as the layers stack up — pulling in context, resolving pronouns, binding objects to verbs — until the last word knows enough to predict what comes next.

Click a layer to step through; click a word to see exactly which earlier words contributed and what they added.
