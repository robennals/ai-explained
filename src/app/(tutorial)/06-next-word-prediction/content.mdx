...
Here's the idea: instead of a lookup table, use a neural network. Feed it the *embeddings* of the previous few words (from Chapter 4), pass them through some layers, and have it output a prediction for the next word.
...
Next-word prediction is also a remarkably powerful training signal, because it lets us extract knowledge from everything humans have ever written. Every sentence ever published becomes a training example. Every Wikipedia article, every novel, every scientific paper teaches the model something about how the world works — all through the lens of "what word comes next?"

That said, modern AI systems don't stop at next-word prediction. In later chapters we'll see techniques that push far beyond it: reinforcement learning from human feedback (teaching models to be helpful rather than just fluent), chain-of-thought reasoning (letting models think step by step through hard problems), and self-play (where models improve by competing against themselves). Next-word prediction is the foundation, but the building has many floors.
...
<TryItInPyTorch notebook="06-next-word-prediction">
