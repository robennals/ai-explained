...
<Lead>
Your brain is a neural network. GPT, Claude, and every other modern AI system are also neural networks, built from artificial versions of the same idea. They can write essays, generate images, translate languages, and hold conversations. You might expect the technology behind all of this to be fearsomely complex. It isn't. It's remarkably simple.
</Lead>
...
<KeyInsight>
**One neuron = one straight line through input space.** It can separate points that fall on opposite sides of a line (AND, OR) but not patterns that require curves (XOR). Adding layers lets neurons combine their lines into complex boundaries — which is why deep networks can learn anything. And because every weight in the network contributes to the error, every weight has a gradient we can follow — so gradient descent can optimize the whole network at once.
</KeyInsight>
...
At the extreme, a very sharp neuron acts like a logic gate — snapping between 0 and 1 with almost nothing in between. This might sound desirable, but it's actually a problem for training. Remember: gradient descent needs smooth slopes to follow. When the sigmoid is nearly flat on both sides of a sharp step, the gradient almost vanishes — the optimizer can't tell which direction to move. This is why real networks use **regularization** techniques that gently discourage weights from growing too large, keeping the neurons in the smooth, trainable middle ground.

But having the right architecture isn't enough. You also need to find the right weights — and in a real network there are millions of them. The algorithm that computes all the gradients at once is called **backpropagation**. It works through the network from output back to input, nudging every weight in the right direction simultaneously.

## Training Deep Networks

We've seen that one neuron draws a single line, and two neurons can combine their lines to solve XOR. What happens when we keep going? Each neuron you add contributes another boundary. Each layer you add lets the network combine boundaries from the previous layer into more complex shapes. With enough neurons and layers, a neural network can carve up input space into **any** pattern — it can compute any function you can imagine. This is the **universal approximation theorem**, and it's why neural networks are so powerful.

The good news is that even as networks get bigger, they stay trainable. Every weight in the network contributes to the final output, which means every weight contributes to the error. And if it contributes to the error, it has a gradient — a direction we can nudge it to make the error smaller. Backpropagation computes all these gradients at once, flowing the error signal backward from the output through every layer, so that every weight gets updated simultaneously. A network with ten thousand weights is no harder to train *in principle* than one with ten — it just takes more computation.
