...
<Lead>
Every chapter in this tutorial has an optional "Try it in PyTorch" section at the bottom. This appendix gets you set up so you can run that code, and teaches you enough PyTorch to understand what it's doing.
</Lead>
...
Neural network layers, optimizers, and data loaders are all built on top of those two primitives.
...
Every "Try it in PyTorch" section in this tutorial links directly to a Colab notebook. Click the link and you're running code, with no setup or installation.
...
4. The notebook runs on Google's servers, and your computer just shows the results

Colab is the easiest way to get started. If you just want to run the tutorial notebooks, you can skip the rest of this section entirely.

## Installing PyTorch Locally

If you want PyTorch on your own machine, maybe to experiment beyond the notebooks or to work in your own editor, here's how.
...
That's the CPU-only version, which is fine for everything in this tutorial. PyTorch can also use your GPU to run tensor math much faster:
...
- *Apple Silicon Macs*: if you have an M1/M2/M3/M4 Mac, PyTorch can use the GPU via Apple's MPS (Metal Performance Shaders) backend. It works out of the box with the standard `pip install torch`, with no extra setup. Move tensors to the GPU with `x.to("mps")`.

## Tensors

A *tensor* is PyTorch's word for "a box of numbers." Every piece of data in PyTorch is a tensor.

### A Quick Python Primer

A few bits of Python syntax you'll see everywhere:

*Decimal points make numbers precise.* Writing `3.0` instead of `3` tells Python this is a decimal number (a "float"), not a whole number (an "integer"). AI math needs decimal precision, with values like `0.7` or `-1.234`, so you'll see that trailing `.0` a lot. Writing `1.` is shorthand for `1.0`.
...
For *matrix multiplication* (the workhorse of neural networks: every layer of weighted sums in [Chapter 3](/neurons) is a matrix multiply under the hood), use `@`:
...
If you know calculus, a gradient is a derivative. If you don't, no problem. Think of it as: "how sensitive is the output to a small change in this input?" If the gradient is large, a small nudge to the input causes a big change in the output. If it's near zero, that input barely matters.

Computing gradients by hand for millions of parameters would be impractical. PyTorch does it automatically.
...
You almost never call `.backward()` directly. PyTorch's optimizers handle it. Understanding that it exists, and that it works by tracking operations, helps you debug when things go wrong.
...
The most basic layer. It computes `y = x @ W.T + b`, a weighted sum of the inputs plus a bias, applied to a whole layer of neurons at once (see [Chapter 3](/neurons)):
...
This is a three-layer neural network. *ReLU* ("rectified linear unit") is just `max(0, x)`. It passes positive values through unchanged and clamps negative values to zero. It plays the same role as the sigmoid in [Chapter 3](/neurons): a nonlinear activation function applied after each weighted sum. Most modern networks use ReLU instead of sigmoid because it's faster and easier to train. The activations between linear layers are essential. Without a nonlinearity in the middle, two stacked linear layers collapse mathematically into a single linear layer, no matter how many you stack.
...
Five lines, repeated thousands of times. That's how every neural network is trained, from a toy example to GPT-4. The model, loss function, and optimizer change, but the loop stays the same.
...
Time to train a network to learn XOR, a problem that a single neuron can't solve (see [Chapter 3](/neurons)) but a two-layer network can.
...
- `nn.Sigmoid()` squashes the output to the 0–1 range, which is what you want for a yes/no answer.
...
You now know enough PyTorch to follow every notebook in this tutorial. From here:
