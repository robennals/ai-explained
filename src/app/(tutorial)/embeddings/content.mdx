...
Start with the classic: king &minus; man + woman = queen. Then try the other presets — prince, father, boy, son, husband. They all transfer the same male-to-female relationship. Now try your own: pick any word for A, something it shares a relationship with for B, and the other end of that relationship for C. What analogies can you find? What happens when you try one that doesn't make sense?
</EmbeddingArithmeticWidget>

Not every analogy works. Try subtracting "cat" from "dog" and adding "piano" — you'll get other instruments, not something meaningful about dogs. The embedding only learned relationships that were useful for its training task. Gender shows up as a consistent direction because it matters for predicting word usage. But "dog-ness minus cat-ness" isn't a relationship the embedding had any reason to learn.
...
Why does addition work at all? Because the embedding has learned to represent certain relationships as *consistent directions*. If gender is roughly the same direction everywhere in the space — the same offset from king to queen as from man to woman, from prince to princess, from boy to girl — then subtraction isolates that direction, and addition applies it. Mathematicians call this **linear structure**, and it's remarkably useful.
