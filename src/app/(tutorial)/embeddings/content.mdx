...
The embedding doesn't just group similar words together — *directions* in the space carry meaning too. The direction from "tiny" to "huge" captures size. The direction from "boy" to "man" captures age. The direction from "salad" to "cake" captures savory-to-sweet. Within regions where the embedding has learned useful structure, the line between two words defines a meaningful spectrum.

The widget below tries to find words that genuinely lie on this direction — not just words that are similar to one endpoint, but words whose embedding sits close to the *line* connecting both. Some pairs of words produce a rich, cleanly-ordered spectrum. Others produce only a handful of matches. And some — pairs the embedding never had a reason to organize together — produce nothing meaningful in between.

<WordPairSpectrumWidget>
Try "tiny ↔ huge" first — a long, clean size gradient because both endpoints differ only in size, so the direction is pure. Try "salad ↔ cake" for a savory-to-sweet food spectrum. Then try "cottage ↔ palace" — you'll get only a handful of buildings; the embedding has surprisingly few words that lie cleanly on this direction. Try "pond ↔ ocean" — lakes and rivers appear, but also "coral," because the direction is bundled: water-body size *and* aquatic context together. Then try "man ↔ sausage" — words pile up near each endpoint with empty space in the middle, because there's no learned direction connecting people and sausages. The widget shows whatever the embedding learned, no padding.
</WordPairSpectrumWidget>

This is why unlabeled dimensions are powerful: concepts like size and age exist as directions in the space even though no dimension is labeled. But here's the catch — each direction encodes whatever bundle of features happens to vary between the two endpoints in the training corpus, not necessarily a single clean concept. Pick endpoints that differ in only one obvious way (`tiny` and `huge` are both pure size adjectives) and the direction is pure. Pick endpoints that differ in many ways and the line collects whatever shares that bundle. The embedding is rich in local structure, and that structure is shaped by what the model was trained on — not by some idealized geometry of meaning.

<KeyInsight>
In a well-trained embedding, directions carry meaning — but a direction encodes whatever bundle of features varies between two words in the training corpus, not necessarily a single clean concept. Pick words that differ in only one way and you get a pure spectrum. Pick words that differ in many ways and the line encodes the whole bundle.
</KeyInsight>
