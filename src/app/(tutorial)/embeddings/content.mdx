An AI has to represent the meaning of a word as a list of numbers, a [vector](/vectors). The obvious approach is to decide in advance what each number measures, but that falls apart fast. Words refer to everything, and no fixed set of properties describes colours, emotions and countries at the same time. The trick is to put similar things close to each other, and allow the same direction to mean different things in different places.
