An AI has to represent the meaning of each word as a list of numbers, a vector. The obvious approach is to decide in advance what each dimension of the vector measures (e.g. size, aliveness, scariness), but that falls apart fast. A word can refer to absolutely anything, and no fixed set of properties describes colours, emotions and countries at the same time. The trick is to abandon the idea that the dimensions of a vector should have fixed meanings, and instead just put similar things close to each other and allow the same direction to mean different things in different places.
...
This is how modern AI builds up complex meaning. In <Ch slug="transformers" />, we'll see that each layer of a transformer model adds vectors to deepen the model's understanding of what each word in a text is about.
...
Each connection carries a weight. Since only one input word is "on," the output of the first layer is just the weights for that one word. Those weights *are* the embedding. Unlike the neurons we saw in <Ch slug="neurons" />, the embedding layer has no bias and no activation function.
