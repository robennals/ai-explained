An AI has to represent the meaning of a word as a list of numbers, a [vector](/vectors). The obvious approach is to decide in advance what each number measures, but that falls apart fast. Words refer to everything, and no fixed set of properties describes colours, emotions and countries at the same time. The trick is to let the model invent its own dimensions instead. This chapter is about how that works, and what the resulting space of meanings looks like from the inside.
